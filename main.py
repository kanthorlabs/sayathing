...
class ServiceManager:
    ...
    async def _run_http_server(self):
        ...
        try:
            ...
            await asyncio.wait(
                [server_task, asyncio.create_task(self.shutdown_event.wait())], return_when=asyncio.FIRST_COMPLETED
            )
        finally:
    ...
    async def _run_primary_worker(self, worker_id: str):
        ...
        try:
            ...
            await asyncio.wait(
                [worker_task, asyncio.create_task(self.shutdown_event.wait())], return_when=asyncio.FIRST_COMPLETED
            )
        finally:
    ...
    async def run(self):
        ...
        self.logger.info("Starting SayAThing service manager...")
        self.logger.info(
            f"Configuration: HTTP={self.enable_http}, Primary Workers={self.primary_workers}, Retry Workers={self.retry_workers}"
        )
...
Examples:
  ...
  python main.py --primary-workers 4 --retry-workers 2
    ...
    parser.add_argument("--no-http", action="store_true", help="Disable HTTP server (default: enabled)")

    # Worker configuration
    parser.add_argument(
        "--primary-workers", type=int, default=1, help="Number of primary workers to spawn (default: 1, minimum: 0)"
    )
...
async def main():
    ...
    service_manager = ServiceManager(
        enable_http=enable_http, primary_workers=primary_workers, retry_workers=retry_workers
    )
