...
class RetryWorker:
    ...
    def __init__(
        self,
        worker_id: str = f"retry-{time.time()}",
            ...
            while self.is_running:
                try:
                    ...
                    retry_tasks = await self.queue.retry(
                        size=self.batch_size, visibility_timeout=self.visibility_timeout, max_attempts=self.max_attempts
                    )
    ...
    async def _claim_task_for_processing(self, task_id: str):
        ...
        try:
            ...
            await self.queue._update_task_state(task_id, TaskState.PROCESSING, expected_state=TaskState.PENDING)
        except Exception as e:
        ...
        try:
            ...
            for item in task.items:
                try:
                    ...
                    self.logger.debug(
                        f"Generated audio for retry task {task.id}, size: {len(tts_response.audio)} bytes"
                    )
