...
def create_sample_task(task_id: str = "", text: str = "Test retry message") -> Task:
    ...
    task_item = TaskItem(
        request={"text": text, "voice_id": "kokoro.af_heart", "metadata": {"test": True, "retry_test": True}},
        response_url="",
    )
...
async def test_retry_worker():
    ...
    try:
        ...
        tasks = [
            create_sample_task("retry-task-1", "Hello world retry test"),
            create_sample_task("retry-task-2", "Another retry test message"),
        ]
