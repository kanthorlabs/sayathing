...
from worker import QueueConfig, Task, TaskItem, TaskState
...
async def test_queue_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create a test task
        task_item = TaskItem(
            request=TextToSpeechRequest(text="Test persistence message", voice_id="kokoro.af_heart"),
        ...
        logger.info(
            "Successfully retrieved persisted task: %s (state: %s)", retrieved_task.id, retrieved_task.state.name
        )
