...
from worker import (InvalidStateTransitionError, QueueConfig, Task, TaskItem,
                    TaskNotFoundError, TaskState)
...
def create_sample_task(task_id: str = "", text: str = "Test message") -> Task:
    ...
    task_item = TaskItem(
        request=TextToSpeechRequest(text=text, voice_id="kokoro.af_heart", metadata={"test": True}),
        response_url="https://example.com/webhook",
    )

    return Task(id=task_id, state=TaskState.PENDING, schedule_at=0, items=[task_item], created_at=0, updated_at=0)
...
async def test_enqueue_dequeue(worker_queue):
    ...
    # Create test tasks
    tasks = [create_sample_task(text=f"Test message {i}") for i in range(3)]
...
async def test_cancel_and_resume(worker_queue):
    ...
    tasks = [create_sample_task(text="Cancel test"), create_sample_task(text="Resume test")]
